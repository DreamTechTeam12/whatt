...
client.once('ready', async () => {
    ...
    client.user.setPresence({ activities: [{ name: 'Geliştirilme Aşamasındayım Ama Kullanabilirsiniz!' }] });
...
client.on("messageCreate", async (message) => {
    if (client.kufurEngel && !message.author.bot) {
        ...
        if (regex.test(message.content)) {
            ...
            try {
                await message.member.timeout(10 * 60 * 1000, "Kimse Kusura Bakmasın Ama Susturulmuştur!"); // 10 dakika (600000 ms)
                message.channel
                    .send(`${message.author}, bu sunucuda küfür etmek yasaktır! 10 Dakikalik Mutenin Tadını Çıkar!`)
                    .then((msg) => {
        ...
        if (Reklamlist.some(reklam => message.content.toLowerCase().includes(reklam))) {
            try {
                ...
                if (message.member.permissions.has(PermissionsBitField.Flags.ModerateMembers)) {
                    ...
                    await message.channel.send(`${message.author} kullanıcısına reklam yaptığı için 5 dakika süreyle zamanaşımı uygulandı.`);
                } else {
                    await message.channel.send(`${message.author}, reklam yaptığınız için zamanaşımı uygulanamadı. Lütfen yetkili birine başvurun.`);
                }
            } catch (error) {
...
client.on('messageCreate', async (message) => {
    ...
    if (client.afk.has(message.author.id)) {
        ...
        const afkEmbed = new EmbedBuilder()
            .setDescription(`${message.author.tag}, AFK modundan çıktın, Günaydınn! :D`)
            .setColor('#ffcc00');
...
client.on('guildMemberUpdate', async (oldMember, newMember) => {
    ...
    if (oldMember.communicationDisabledUntil !== newMember.communicationDisabledUntil) {
        ...
        if (newMember.communicationDisabledUntil) {
            ...
            description = `🔊 **@${newMember.user.tag}** kullanıcısının susturulması kaldırıldı. Artık konuşabilirsin!`;
        }
...
client.on('guildBanAdd', async (ban) => {
    ...
    const embed = new EmbedBuilder()
        ...
        .setDescription(`🚫 **@${ban.user.tag}** kullanıcısı sunucudan yasaklandı Güle güleeeeee!! 😀.`)
        .setColor('#ff0000')
...
client.on('guildMemberKick', async (member) => { 
    ...
    const embed = new EmbedBuilder()
        .setAuthor({ name: member.user.tag, iconURL: member.user.displayAvatarURL({ dynamic: true }) })
        .setDescription(`👢 ${member.user.tag} kullanıcısı sunucudan roketle uzaya fırlatıldı. Uzaylı dostlara merhaba dese iyi olur :D `)
        .setColor('#2C2F33')
        .setFooter({ text: `${member.guild.name} • ${new Date().toLocaleString()}`, iconURL: member.guild.iconURL({ dynamic: true }) })
        .setTimestamp();
